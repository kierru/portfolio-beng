const ResumeCard = ({ title, subTitle, result, href}: any) => {
  const handleClick = () => {
    window.open(href, '_blank'); // This will navigate to the specified URL
  };
  return (
    <div className="w-full h-1/2 group flex">
      ...
      <div className="w-full bg-black bg-opacity-20 hover:bg-opacity-30 duration-300  rounded-lg p-4 lgl:px-10 flex flex-col justify-center gap-6 lgl:gap-10 shadow-shadowOne">
        <div className="flex flex-col lgl:flex-row justify-between gap-4 lgl:gap-0 lgl:items-center">
          ...
          <div>
            <img
              className="w-40 h-40 group-hover:scale-110 duration-300 cursor-pointer"
              onClick={handleClick}
              src={result}
              alt="src"
            />
